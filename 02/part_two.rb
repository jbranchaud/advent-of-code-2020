...
Rule = Struct.new(:letter, :position1, :position2)

# parsing of the input strings
def parse_input(input_string)
  # "16-20 j: vjkjjcjjrjjmtnbjjjnj"
  unparsed_rule, password = input_string.split(': ')
  position1_position2, letter = unparsed_rule.split(' ')
  position1, position2 = position1_position2.split('-').map(&:to_i)

  rule = Rule.new(letter, position1, position2)

  [password, rule]
end

# validation of the passwords based on the rules
def validate_password(input_string)
  password, rule = parse_input(input_string)

  # vjkjjcjjrjjmtnbjjjnj
  letter_at_position1 = password[rule.position1 - 1]
  letter_at_position2 = password[rule.position2 - 1]

  (letter_at_position1 == rule.letter) ^ (letter_at_position2 == rule.letter)
end

answer =
  inputs.filter(&method(:validate_password)).size
